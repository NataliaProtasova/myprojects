...
class ProductUpdateTestCase(APITestCase):
    def test_update_product(self):
        ...
        self.assertEqual(updated.name, 'New Product1')

    def test_upload_product_photo(self):
        product = Product.objects.first()
        original_photo = product.photo
        photo_path = os.path.join(settings.MEDIA_ROOT, 'products', 'prod2.jpg')
        ...
        try:
            updated = Product.objects.get(id=product.id)
            expected_photo = os.path.join(settings.MEDIA_ROOT, 'products', 'prod2')
